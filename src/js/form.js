...
const createTitle = () => {
  ...
  const p = createElement('p', { className: 'msg-info', innerHTML: 'Tous les champs sont obligatoires.<br/>Vous pouvez effacer vos données personnelles en supprimant de votre navigateur les cookies de ce site.' })
  return [h2, p]
}
...
const createMoveField = (moveData) => {
  const formMoveAttrs = { className: 'form-radio align-items-center' }
  const formMove = createElement('div', formMoveAttrs)
  const appendToMove = appendTo(formMove)

  const id = `radio-${moveData.code}`
  const inputMoveAttrs = {
  ...
  const inputMove = createElement('input', inputMoveAttrs)

  const labelAttrs = { innerHTML: moveData.label, className: 'form-radio-label', for: id }
  const label = createElement('label', labelAttrs)

  appendToMove([inputMove, label])
  return formMove
}

const createMoveFieldset = (movesData) => {
  const fieldsetAttrs = {
    id: 'move-fieldset',
    className: 'fieldset',
  }
  ...
  const movesFields = movesData.items.map(createMoveField)

  appendToFieldset([...movesFields])
  // Créer un form-radio par motif
  return fieldset
}
...
export function createForm () {
  ...
  const movesData = formData
    .flat(1)
    .find(field => field.key === 'move')
  const moveFieldset = createMoveFieldset(movesData)
  const formFirstPart = formData
    .flat(1)
    .filter(field => field.key !== 'move' && field.key !== 'reason')
  ...
  appendToForm([...createTitle(), moveFieldset, ...formFirstPart, reasonFieldset])
}
